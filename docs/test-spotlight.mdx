...
This tutorial guides you on using our APIs to place a regular order with all responsibilities centralized under a single account. Follow these steps to accomplish the process smoothly. First, simulate a cart to explore available delivery and payment options. Next, check if the customer already exists in your database by verifying their email adress. Assemble the cart by organizing all essential order information correctly. Then, place the order on the VTEX platform. Notify VTEX with the payment information to proceed. Finally, request order processing details from the payments module and conclude the process by checking the result.
...
> ✅ To navigate through this article and explore specific topics, click the steps below. Relevant code will be highlighted and focused for your convenience.
...
### Step 2 - Check if the customer already exists in your database
...
If the response to this request returns any content, it means that you already have this customer’s information on your store’s database. In this case, you can use that information in the next steps without having to ask the shopper to provide it to you again.
...
If the delivery method is pickup point, add the information `"selectedDeliveryChannel": "pickup-in-point"`, as in this example.
...
>❗ Make sure that all value-related fields match the information sent in step 4, when placing the order.
