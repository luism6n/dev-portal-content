...
1. [Simulate a cart](#1-simulate-a-cart): Find out the available delivery and payment options.

1. [Check if the customer already exists in your database](#2-check-if-a-customer-already-exists-in-your-database): Ensure the order can be placed by checking if the customer email already exists in your database.

2. [Assemble a cart](#3-assemble-a-cart): Assemble all necessary order information in the correct format.

3. [Place the order](#4-place-the-order): Place the order on the VTEX platform.

4. [Send payment information](#5-send-payment-information): Notify VTEX that you have payment information ready to proceed.

5. [Request order processing](#6-request-order-processing): Request order processing information from the payments module.
